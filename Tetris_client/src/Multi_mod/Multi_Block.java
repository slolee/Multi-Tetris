...
public class Multi_Block {
	...
	public void spin() {
		...
		int distance2 = 999;
		for(int i=0; i<4; i++) {
			if(temp_y[i] > 20*Multi_Data.myBlockSize) { //3. 맨밑을 넘어가면 안되는거다.
				return;
			}else if(temp_x[i] < 0) { //1. 왼쪽맵을 벗어났는지 확인
				if(distance < (-1 * temp_x[i])) //얼마나 벗어났는지를 확인해서 그만큼 이동거리에 저장
					distance = -1 * temp_x[i];
			}else if(temp_x[i] > 9*Multi_Data.myBlockSize) { //2. 오른쪽맵을 벗어났는지 확인
				if(distance2 > -1 * (temp_x[i] - 9*Multi_Data.myBlockSize))
					distance2 = -1 * (temp_x[i] - 9*Multi_Data.myBlockSize); //얼마나 벗어났는지를 확인해서 그만큼 이동거리에 저장
	...
	public void down_Block() {
		if(this.currrent_block == 9) { //현재 블럭이9면
			...
			for(int i=0; i<4; i++) { //만약 y좌표가 그 도형에서 최대값, 즉, 가장밑에있는 도형들에 한해서
				if(this.current_shape[1][i] == max_y) {
					if(max_y == 20*Multi_Data.myBlockSize) { //맨 바닥이랑 y좌표가 같으면 map에 그 색을 저장하고, current_block을 9로 설정해서 newBlock()함수 호출.
